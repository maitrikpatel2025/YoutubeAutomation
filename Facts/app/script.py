...
def generate_youtube_prompt(idea,logger):
    ...
    Finally, conceptualize six creative image ideas based on the description. The images should be informative, detailed, and artistically modern. They should include elements nature, history, technology, culture impact of your visuals, making them more engaging and informative for the audience abstract forms to give your images a modern touch. The primary objective is to inspire and educate the audience while fostering a deeper appreciation of the context.
    Topic: {idea}
...
def generate_script(idea,logger):
    ...
    script= f"""
    Create a short, engaging voice-over script for a video on platforms like TikTok, or Instagram Reels. The script should begin with "Hello" and it Start with an attention-grabbing question related to the topic and presented individually top 5 facts , starting with Fact 5 and progressing to Fact 1. For each fact, the host shares an intriguing piece of information and provides additional context or details. The script highlights the unique qualities, historical relevance, remarkable characteristics, unusual phenomena, and impressive achievements of the topic. Finally, the conclusion wraps up the video.
    DON'T MAKE ME REPEAT MYSELF. I NEED A SCRIPT THAT'S MORE THAN 200 WORDS BUT LESS THAN 240 WORDS and be suitable for a general audience.
    Use simple language and avoid jargon to ensure accessibility.
    Keep the script concise and avoid unnecessary details or tangents or shot details .
    {idea} 
    """
...
def generate_image_prompt(description):
    ...
    Your task is to create a single image prompt for a generative AI titled "Midjourney". This prompt will provide the AI with specific details to generate an image.They should include elements nature, history, technology, culture impact of your visuals, making them more engaging and informative for the audience abstract forms to give your images a modern touch. The primary objective is to inspire and educate the audience while fostering a deeper appreciation of the context. Here are the necessary steps and guidelines:
    {description}
